...
let handlechange=(id)=>{
  ...
  const updatedata=async()=>{
        const fetchingdata=await fetch(`http://localhost:3500/items/${id}`,{
            method:'PATCH',
            headers:{
  ...
  if(additem.length&&today){
    ...
    setitems(arrdata)
      ...
      let newadddata=async()=>{
          let addingdata=await fetch("http://localhost:3500/items",{
              method:'POST',
              headers:{
                  'Content-Type':'application/json'
              },
...
let handledelete=(id)=>{
      ...
      let deldata=async()=>{
        let deletingdata=await fetch(`http://localhost:3500/items/${id}`,{
            method:'DELETE'
        })
      }
      deldata()
